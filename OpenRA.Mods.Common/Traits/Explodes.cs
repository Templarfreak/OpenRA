		...
		void INotifyKilled.Killed(Actor self, AttackInfo e)
		{
			...
			if (Info.Type == ExplosionType.Footprint && buildingInfo != null)
			{
				...
				foreach (var c in cells)
					weapon.Impact(Target.FromPos(self.World.Map.CenterOfCell(c)), Target.FromPos(self.World.Map.CenterOfCell(c)), source, Enumerable.Empty<int>());

				return;
			}

			// Use .FromPos since this actor is killed. Cannot use Target.FromActor
			weapon.Impact(Target.FromPos(self.CenterPosition), Target.FromPos(self.CenterPosition), source, Enumerable.Empty<int>());
		}
